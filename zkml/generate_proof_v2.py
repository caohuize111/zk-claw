"""
ZK-Claw: EZKL full pipeline for model_v2 (in-circuit normalization)
Input: raw sensor values -> normalization -> MLP -> decision
"""
...
BASE = SCRIPT_DIR
ARTIFACTS = os.path.join(BASE, "artifacts_v2")
...
MODEL = os.path.join(BASE, "model_v2.onnx")
INPUT = os.path.join(BASE, "input_v2.json")
...
VERIFIER_SOL = os.path.join(SCRIPT_DIR, "..", "contracts", "contracts", "verifier", "Halo2VerifierV2.sol")


async def main():
    print("=" * 60)
    print("ZK-Claw EZKL Pipeline V2 (in-circuit normalization)")
    print("=" * 60)
    ...
    print(f"  Model: {MODEL}")
    print(f"  Input: raw sensor values (no pre-normalization)")
